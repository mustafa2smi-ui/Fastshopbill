...
function calculate() {
    try {
        ...
        // State save karein undo ke liye
        lastState = { history: [...historyItems], total: grandTotal };
        ...
        // Purana format wapas: Name: Exp = Result
...
async function printBill() {
    ...
    try {
        ...
        esc += '--------------------------------\n';
        esc += `SubTotal: Rs.${grandTotal.toFixed(2)}\n`;
        esc += `Prev Due: Rs.${prevDue.toFixed(2)}\n`;
        esc += `Received: Rs.${received.toFixed(2)}\n`;
        esc += '--------------------------------\n';
        esc += '\x1B\x21\x10' + `NET BAL:  Rs.${net.toFixed(2)}\n` + '\x1B\x21\x00';
        esc += '\x1B\x61\x01' + '\nDHANYAWAD! VISIT AGAIN\n\n\n\n';
...
// ... baaki addNumber, addOperator functions pehle jaise hi rahenge ...
function setCurrentInput(field) {
    activeField = field;
    document.querySelectorAll('.input-group input').forEach(el => el.style.borderColor = "#ccc");
    document.getElementById(field).style.borderColor = "#007bff";
